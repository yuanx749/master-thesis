...
centroids = np.array([adata.X[adata.obs['spage2vec'] == i].mean(axis=0) for i in range(num_clusters)])
...
df_feat = pd.DataFrame(data=np.zeros((data.num_features, num_clusters)), index=df_taglist['gene'])
...
# sns.heatmap(df_feat.sort_index(), cmap='viridis', xticklabels=True, yticklabels=True, ax=ax)
# fig.savefig(os.path.join(fig_dir, 'explainer-sup.png'))
g = sns.clustermap(
    df_feat.sort_index(), method='average', metric='correlation', row_cluster=False,
