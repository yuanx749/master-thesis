...
es = keras.callbacks.EarlyStopping(
    monitor="loss", min_delta=0, patience=20, restore_best_weights=True
)
...
model_name = model_dir / time.strftime("gcn-%Y%m%d.h5")
...
model_name = model_dir / time.strftime("gcn-embedding-%Y%m%d.h5")
...
node_embeddings = embedding_model.predict(
    fullbatch_generator.flow(g.nodes()), verbose=2
)
