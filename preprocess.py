"""Read data, plot sections figures, create graph, and compute baseline."""
...
for i in range(3):
    for j in range(3):
        ...
        axes[i, j].scatter(spot_x, spot_y, s=1, marker=".", linewidths=0)
...
for name in ["6.5_1", "6.5_2"]:
    ...
    df_cell[name] = df_cell[name].set_index(
        "{}_".format(name) + df_cell[name].index.astype(str)
    )
df_heart_cell = pd.concat(df_cell.values())
cell_types = df_heart_cell["celltype"].dropna().unique()
cell_type_id = [
    (cell_type, int(re.search(r"\((\d+)\)", cell_type).group(1)))
...
# %% [markdown]
# Build network and save the nodes and the edges for downstream.
...
for name in ["4.5_1", "4.5_2", "4.5_3", "6.5_1", "6.5_2", "9.5_1", "9.5_2", "9.5_3"]:
    ...
    df_edge[name] = df_edge[name].set_index(
        "{}_".format(name) + df_edge[name].index.astype(str)
    )
