"""Unsupervised GCN with random walk sampling, implemented using PyTorch Geometric."""
...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, heads=1):
        ...
        self.convs.append(
            GATConv(heads * hidden_channels, hidden_channels, heads=heads, concat=False)
        )
...
def train():
    ...
    z_vn = z_u[neg_idx]
    # z_vn = z_u[torch.randperm(data.num_nodes)]
