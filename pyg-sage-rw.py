"""Unsupervised GraphSAGE, implemented using PyTorch Geometric."""
...
def train(epoch):
    ...
    rw = random_walk(
        data.edge_index[0], data.edge_index[1], node_idx, walk_length=walk_length
    )
    rw_idx = rw[:, 1:].flatten()
    pos_loader = NeighborSampler(
        ...
        node_idx=rw_idx,
        sizes=num_samples,
        batch_size=batch_size * walk_length,
        shuffle=False,
        num_workers=0,
    )
    ...
    neg_loader = NeighborSampler(
        ...
        node_idx=neg_idx,
        sizes=num_samples,
        batch_size=batch_size * num_neg_samples,
        shuffle=True,
        num_workers=0,
    )

    for (batch_size_, u_id, adjs_u), (_, v_id, adjs_v), (_, vn_id, adjs_vn) in zip(
        train_loader, pos_loader, neg_loader
    ):
        ...
        pos_loss = -F.logsigmoid(
            (z_u.repeat_interleave(walk_length, dim=0) * z_v).sum(dim=1)
        ).mean()
        neg_loss = -F.logsigmoid(
            -(z_u.repeat_interleave(num_neg_samples, dim=0) * z_vn).sum(dim=1)
        ).mean()
...
model_name = model_dir / time.strftime("sage-rw-%Y%m%d.pt")
